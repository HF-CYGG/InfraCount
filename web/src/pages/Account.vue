    ...
    <div class="卡片 面板">
      ...
      <form class="表单" @submit.prevent="提交">
        <label class="字段 宽字段">
          ...
          <UiInput v-model="新密码" type="password" autocomplete="new-password" placeholder="请输入新密码" />
        </label>

        <label class="字段 宽字段">
          <div class="字段标题">确认新密码</div>
          <UiInput v-model="确认密码" type="password" autocomplete="new-password" placeholder="请再次输入新密码" />
        </label>

        <div v-if="提示信息" class="提示-次要 小字">{{ 提示信息 }}</div>
        <div v-if="错误信息" class="提示-错误 小字">{{ 错误信息 }}</div>

        <div class="行 行-右 上间距-12">
          <UiButton variant="primary" type="submit" :loading="loading">{{ loading ? "正在提交..." : "确认修改" }}</UiButton>
        </div>
      </form>
    </div>
  </AppLayout>
...
async function 提交(): Promise<void> {
  ...
  try {
    ...
    确认密码.value = "";
    提示信息.value = "密码已修改成功。";
    toastStore.push("密码已修改成功", { tone: "success" });
  } catch (e) {
